...
interface GiftedAvatarProps {
	user: SingleMessageApiResponse | null;
...
export const GiftedAvatar: React.FC<GiftedAvatarProps> = (props) => {
  ...
  const avatarName = useMemo(() => {
	...
	const renderAvatar = useCallback((user: SingleMessageApiResponse) => {
		if (typeof user.director === 'string') {
			return (
				<Img style={styles.avatarStyle}/>
			);
		}
		return null;
	}, []);
